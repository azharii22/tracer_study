         ...
         <div class="container">
            ...
            <div class="row">
               <div class="col-md-8 offset-md-2">
                <form class="forms-sample" action="{{ route('post-data-pengguna-alumni') }}" method="POST">
                    @csrf
                    <div class="form-group">
                    <label for="formGroupExampleInput">Nama</label>
                    <input type="text" class="form-control" name="nama" id="formGroupExampleInput" placeholder="Nama Pengisi">
                    </div>
                    <div class="form-group">
                ...
                {{-- <form class="forms-sample" action="{{ route('post-jawaban-pengguna-alumni') }}" method="POST" enctype="multipart/form-data">
                     <div class="row">
                        @foreach ($kuisionerPenggunaAlumni as $data )
                        <form>
                        <div class="col-md-12">
                        <div class="mt-4"><h4>{{++$i}}. {{$data->pertanyaan}}</h4></div>
                        <input type="radio" id="a" name="jawaban {{$data->id}}" value="jawabanA">
                        <label for="a">{{$data->jawaban_a}}</label><br>
                        <input type="radio" id="b" name="jawaban {{$data->id}}" value="jawabanB">
                        <label for="b">{{$data->jawaban_b}}</label><br>
                        <input type="radio" id="c" name="jawaban {{$data->id}}" value="jawabanC">
                        <label for="c">{{$data->jawaban_c}}</label><br>
                        <input type="radio" id="d" name="jawaban {{$data->id}}" value="jawabanD">
                        <label for="d">{{$data->jawaban_d}}</label><br>
                        <input type="radio" id="e" name="jawaban {{$data->id}}" value="jawabanE">
                        <label for="e">{{$data->jawaban_e}}</label><br>
                        </div>
                        </form>
                        @endforeach
